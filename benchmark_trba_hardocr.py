...
from recog_tools import load_model, recognize_text_from_images
...
HARDOCR_MODEL_PATH = r"model_095.pth"
...
def run_dataset(model_dict: dict, ds_cfg: dict) -> dict:
    print(f"\n### DATASET: {ds_cfg['name']}")
    ...
    for image_name, image_path in zip(image_names, image_paths):
        try:
            with Image.open(image_path) as image:
                pred_list = recognize_text_from_images([image], model_dict)
            predictions[image_name] = pred_list[0] if pred_list else ""
    ...
    result = {
        "dataset": ds_cfg["name"],
        "model": "recog_tools.recognize_text_from_images",
...
def main() -> None:
    ...
    model_dict = load_model(
        ...
        model_path=HARDOCR_MODEL_PATH,
    )

    summary = []
    for ds in DATASETS:
        result = run_dataset(model_dict, ds)
        if result:
