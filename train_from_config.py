...
def train(opt):
    ...
    """ Финальные опции """
    with open(
        f"{opt.save_path}/{opt.exp_name}/opt.txt", "a", encoding="utf8"
    ) as opt_file:
        opt_log = "------------ Options -------------\n"
        args = vars(opt)
    ...
    while True:
        ...
        if (iteration) % opt.valInterval == 0 or iteration == 0:
            ...
            with open(f"{opt.save_path}/{opt.exp_name}/log_train.txt", "a") as log:
                ...
                if current_accuracy > best_accuracy:
                    ...
                    torch.save(
                        model.state_dict(),
                        f"{opt.save_path}/{opt.exp_name}/best_accuracy.pth",
                    )
                    ...
                    torch.save(
                        model.state_dict(),
                        f"{opt.save_path}/{opt.exp_name}/best_valid_loss.pth",
                    )
                    no_improve_counter = 0  # Сброс счетчика
                    improved = True
                else:
                ...
                if no_improve_counter >= opt.no_improve:
                    print(
                        f"No improvement in {opt.no_improve} validation checks, stopping training."
                    )
                    torch.save(
                        model.state_dict(), f"{opt.save_path}/{opt.exp_name}/last.pth"
                    )
                    sys.exit()
        ...
        if (iteration) % 1e5 == 0:
            torch.save(
                model.state_dict(),
                f"{opt.save_path}/{opt.exp_name}/iter_{iteration}.pth",
            )
