...
def evaluate_dataset(
    gt_txt: str, img_dir: str, pipeline: DocumentOCRPipeline, batch_size: int = 1
) -> pd.DataFrame:
    results = []
    total = 0
    ...
    for line in lines:
        ...
        if os.path.exists(img_path):
            samples.append((img_name, gt_text.strip(), img_path))
        else:
    ...
    for i in tqdm(range(0, len(samples), batch_size), desc="Processing batches"):
        ...
        for img_name, gt_text, img_path in batch_samples:
            try:
                ...
                images.append(img)
                meta.append((img_name, gt_text))
            except Exception as e:
                print(f"[ERROR] Failed to open {img_path}: {e}")

        if len(images) == 1:
            # Один образ → обычный метод
            img = images[0]
            w, h = img.size
            try:
                ...
                preds = [word_resp.text]
            except Exception as e:
                print(f"[ERROR] recognize_word failed: {e}")
                preds = [""]
        else:
            try:
    ...
    print(f"[TIME] Evaluation took {elapsed:.2f} seconds")
...
def main():
    ...
    for gt_path, img_path in datasets:
        ...
        df, elapsed = evaluate_dataset(gt_path, img_path, pipeline)
    ...
    with pd.ExcelWriter(output_excel) as writer:
        ...
        final_df[final_df["is_wrong"] == 1].to_excel(
            writer, sheet_name="wrong_preds", index=False
        )
